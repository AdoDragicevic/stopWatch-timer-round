...
Requirements: 
    ...
    - btns passed in as arguments should have an ID (i.e. btn with id="reset" invokes reset method in currenlty running app; buttons wholse 
      ID does not correspond to any of currently running app's methods are not displayed - this allows apps to use different number of btns)
*/
class SwitchBetweenApps {
    ...
    updateDisplay() {
        this.app.inputs.forEach( (input, i) => this.inputs[i].value = this.twoDigitNum( this.app["v" + (i+1)]) );
    };
    ...
    hideExcessInputs() {
        ...
        this.inputs.forEach( (input, i) => input.style.display = this.app.inputs[i] ? "" : "none" );
    }
