class Round extends Time {
    ...
    isSoundOn = this.initSound(false);
    ...
    config = {
        ...
        maxLimit: [0, 0, 99],
        runOnMaxLimit: () => {
            if(this.isDisplayed && this.isSoundOn) this.playSound("bell");
    ...
    runOnStart() {
        let { currRound, isCountingBreakTime, resetBreak, resetTime, time } = this.config; 
        this.config.currRound = currRound || 1;
        this.isDisableInputs = true;
        let totalTime = isCountingBreakTime ? resetBreak : resetTime;
        if( this.areSame(totalTime, time) && this.isSoundOn ) this.playSound("bell");
    }
