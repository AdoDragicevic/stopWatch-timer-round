...
class Time {
    ...
    initSound(bool = false) {
        let sound = document.querySelector(`#${this.name.toLowerCase()} .sound`);
        let soundOnIcon = document.querySelector(`#${this.name.toLowerCase()} .sound__icon--on`);
        let soundOffIcon = document.querySelector(`#${this.name.toLowerCase()} .sound__icon--off`);
        sound.addEventListener("click", () => {
            this.isSoundOn = !this.isSoundOn;
            this.style.toggleSoundIcon(soundOnIcon, soundOffIcon, this.isSoundOn);
        });
        this.style.toggleSoundIcon(soundOnIcon, soundOffIcon, bool);
        return bool;
    }


    playSound(soundName) {
        document.querySelector(`#audio-${soundName}`).play();
    }
